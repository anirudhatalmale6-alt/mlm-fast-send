...
Func _CHOOSECOLOR ( $VRETURNTYPE = 0 , $ICOLORREF = 0 , $IREFTYPE = 0 , $HWNDOWNDER = 0 )
	...
	ElseIf IsArray ( $VRETURNTYPE ) Then
		...
		Else
			Return SetError ( + 4294967291 , 0 , - 1 )
		EndIf
	Else
	...
	If $IRETURNTYPE < 0 Or $IRETURNTYPE > 2 Then
		...
		EndIf
		Return SetError ( + 4294967292 , 0 , - 1 )
	EndIf
	If $IREFTYPE = 1 Then
	...
	If $ARESULT [ 0 ] = 0 Then
		...
		EndIf
		Return SetError ( + 4294967293 , 0 , - 1 )
	EndIf
...
Func _CHOOSEFONT ( $SFONTNAME = "Courier New" , $IPOINTSIZE = 10 , $IFONTCOLORREF = 0 , $IFONTWEIGHT = 0 , $BITALIC = False , $BUNDERLINE = False , $BSTRIKETHRU = False , $HWNDOWNER = 0 )
	...
	If $ACALL [ 0 ] = 0 Then Return SetError ( + 4294967293 , + 4294967293 , - 1 )
...
Global Const $TCN_FOCUSCHANGE = ( $TCN_FIRST + 4294967292 )
Global Const $TCN_GETOBJECT = ( $TCN_FIRST + 4294967293 )
Global Const $TCN_KEYDOWN = ( $TCN_FIRST + 0 )
Global Const $TCN_SELCHANGE = ( $TCN_FIRST - 1 )
Global Const $TCN_SELCHANGING = ( $TCN_FIRST + 4294967294 )
...
Func _GUIIMAGELIST_ADDICON ( $HWND , $SFILEPATH , $IINDEX = 0 , $BLARGE = False )
	...
	_WINAPI_DESTROYICON ( $HICON )
	If $IRET = - 1 Then Return SetError ( + 4294967294 , $IRET , - 1 )
	Return $IRET
EndFunc
...
Global $GDIRROOT = @ScriptDir & "\MLManagerData\"
...
$GGUIMAIN = GUICreate ( "Multilogin Window Manager v2.0" , 358 , 660 , 920 , 180 , BitOR ( $GUI_SS_DEFAULT_GUI , $WS_SIZEBOX , $WS_THICKFRAME ) )
$MAIN = GUICtrlCreateTab ( 8 , 8 , 345 , 589 )
...
$BUTTONDISCORDQUE = GUICtrlCreateButton ( "Send to QUE" , 24 , 196 , 148 , 35 )
$BUTTONSENDTOPROD = GUICtrlCreateButton ( "Send to PROD" , 178 , 196 , 142 , 35 )
...
$LABELQUEWEBHOOK = GUICtrlCreateLabel ( "QUE Webhook:" , 24 , 355 , 90 , 20 )
$INPUTQUEWEBHOOK = GUICtrlCreateInput ( "" , 120 , 355 , 200 , 20 )
$LABELPRODWEBHOOK = GUICtrlCreateLabel ( "PROD Webhook:" , 24 , 385 , 90 , 20 )
$INPUTPRODWEBHOOK = GUICtrlCreateInput ( "" , 120 , 385 , 200 , 20 )
$LABELSHEETURL = GUICtrlCreateLabel ( "Sheet URL:" , 24 , 420 , 70 , 20 )
$INPUTSHEETURL = GUICtrlCreateInput ( "" , 100 , 420 , 220 , 20 )
$LABELSCREENSHOTFOLDER = GUICtrlCreateLabel ( "Save Folder:" , 24 , 450 , 80 , 20 )
$INPUTSCREENSHOTFOLDER = GUICtrlCreateInput ( "" , 110 , 450 , 170 , 20 )
$BUTTONBROWSEFOLDER = GUICtrlCreateButton ( "..." , 285 , 449 , 30 , 22 )
$BUTTONSAVEDISCORD = GUICtrlCreateButton ( "Save Discord Settings" , 100 , 485 , 150 , 30 )
...
$INPUTURL = GUICtrlCreateInput ( "https://www.ticketmaster.com" , 70 , 265 , 180 , 20 )
$BUTTONOPENURL = GUICtrlCreateButton ( "Open URL" , 260 , 260 , 70 , 30 )
...
GUICtrlCreateLabel ( "Dev : ChingChing" , 16 , 600 , 120 , 15 )
...
$INPUTMAINURL = GUICtrlCreateInput ( "https://www.ticketmaster.com" , 16 , 618 , 220 , 20 )
GUICtrlSetResizing ( - 1 , $GUI_DOCKBOTTOM + $GUI_DOCKWIDTH + $GUI_DOCKHEIGHT )
$BUTTONMAINOPENURL = GUICtrlCreateButton ( "Open URL" , 240 , 616 , 65 , 24 )
GUICtrlSetResizing ( - 1 , $GUI_DOCKBOTTOM + $GUI_DOCKWIDTH + $GUI_DOCKHEIGHT )
...
Local $IEXLISTVIEWSTYLE = BitOR ( $LVS_EX_FULLROWSELECT , $LVS_EX_SUBITEMIMAGES , $LVS_EX_GRIDLINES , $LVS_EX_DOUBLEBUFFER , $LVS_EX_CHECKBOXES )
...
Func GETBROWSERS ( )
	$SNEEDSSORT = False
	Local $ALISTFF = WinList ( "[REGEXPTITLE:.*Stealthfox$;CLASS:MozillaWindowClass;]" )
	Local $ALISTCH = WinList ( "[REGEXPTITLE:.*Mimic$;CLASS:Chrome_WidgetWin_1;]" )
	_ARRAYCONCATENATE ( $ALISTFF , $ALISTCH , 1 )
	$ALISTFF [ 0 ] [ 0 ] += $ALISTCH [ 0 ] [ 0 ]
	For $I = 1 To $ALISTFF [ 0 ] [ 0 ]
		$STITLE = $ALISTFF [ $I ] [ 0 ]
		$SHANDLE = $ALISTFF [ $I ] [ 1 ]
		If StringInStr ( $STITLE , "Stealthfox" ) Then
			$STRIMRIGHT = 13
			$SBROWSERICON = 1
		Else
			$STRIMRIGHT = 8
			$SBROWSERICON = 0
		EndIf
		$STAB = StringTrimRight ( StringMid ( $STITLE , StringInStr ( $STITLE , ":" ) + 1 ) , $STRIMRIGHT )
		...
		If $ALISTFF [ $I ] [ 0 ] <> "" And BitAND ( WinGetState ( $SHANDLE ) , 2 ) Then
			...
			If $SSEARCH = - 1 Then
				; Try to get profile name from window title first (before the colon)
				; Window title format: "ProfileName: Tab Title - Mimic" or "ProfileName: Tab Title - Stealthfox"
				$SPROFILE = ""
				Local $iColonPos = StringInStr ( $STITLE , ":" )
				If $iColonPos > 0 Then
					$SPROFILE = StringStripWS ( StringLeft ( $STITLE , $iColonPos - 1 ) , 3 )
				EndIf
				; If not found in title, try command line
				If $SPROFILE = "" Then
					Local $IPID = WinGetProcess ( $SHANDLE )
					$SCOMMANDLINE = GETCHROMECOMMANDLINE ( $IPID )
					; Try quoted pattern first (handles spaces in name)
					Local $SPATTERN = '--session_name="([^"]+)"'
					Local $ARESULT = StringRegExp ( $SCOMMANDLINE , $SPATTERN , 1 )
					If Not @error Then
						$SPROFILE = StringStripWS ( $ARESULT [ 0 ] , 3 )
					Else
						; Fallback to unquoted pattern
...
Func BROWSERTOOLBARCREATE ( )
	$GGUITOOLBAR = GUICreate ( "" , 160 , 25 + 4294967293 , 0 , 0 , $WS_POPUP , BitOR ( $WS_EX_TOPMOST , $WS_EX_TOOLWINDOW , $WS_EX_CLIENTEDGE ) )
...
Func GUICHECKCONTROLS ( )
	...
	Case $GUI_EVENT_CLOSE
		; Unregister all AdLib functions first to prevent hang
	...
	Case $BUTTONDISCORDQUE
		SENDDISCORDSCREENSHOT ( "que" )       ; 1. Takes screenshot + sends to QUE instantly
	Case $BUTTONSENDTOPROD
		SENDDISCORDSCREENSHOT ( "prod" )      ; 2. Takes screenshot + sends to PROD instantly (no save)
	Case $BUTTONSCREENSHOTPROD
		SAVESCREENSHOTONLY ( "prod" )          ; 3. Saves screenshot locally only, no Discord
	Case $BUTTONSCREENSHOTSENDPROD
		SENDPRODTODISCORD ( )                 ; 4. Sends all saved screenshots from today's prod folder
	...
	Case Else
		; Check main tab group buttons A-Z
		...
		For $iMG = 0 To 25
			...
			EndIf
		Next
		; Check positioner tab group buttons A-Z
	...
	; Handle ListView double-click - pop up and maximize the profile window
	If $GLISTVIEWCLICKED Then
		$GLISTVIEWCLICKED = False
		BROWSERACTION ( "show" )
	EndIf

	; Handle ListView selection change - pop up the selected profile window
	; Skip if BROWSERMOVE or SORTING is handling it (prevents double handling and blinking)
	If Not $GBROWSERMOVEINPROGRESS And Not $GSORTINGINPROGRESS Then
		...
		If IsArray ( $aSelNow ) And $aSelNow[0] = 1 Then
			...
			If $iCurrentSel <> $GLASTSELECTEDINDEX Then
				; Check if mouse is over ListView header (sorting) - skip if so
				Local $aMousePos = MouseGetPos ( )
				Local $aGUIPos = WinGetPos ( $GGUIMAIN )
				If IsArray ( $aGUIPos ) Then
					; Header is at Y=72 relative to GUI, about 25px tall
					...
					If $aMousePos[1] >= $iHeaderTop And $aMousePos[1] <= $iHeaderBottom Then
						; Mouse is over header - this is a sort click, skip popup
				...
				If $sHwndSel <> "" Then
					...
					If $hWndSel <> 0 And WinExists ( $hWndSel ) Then
						; Minimize all other profile windows first (only if checkbox is checked)
...
Func BROWSERMOVE ( $SDIRECTION )
	...
	If $SCOUNT = 0 Then
		...
		Return
	EndIf
	; Get currently selected item instead of using global position
...
Func BROWSERACTION ( $SACTION , $SALL = False )
	...
	CURRENTPOSITIONUPDATE ( )
EndFunc
; Close only checked/marked profiles
Func CLOSEMARKEDPROFILES ( )
	Local $iCount = _GUICtrlListView_GetItemCount ( $LISTVIEW1 )
	If $iCount = 0 Then Return

	; Collect checked indices
	Local $aChecked[$iCount]
	Local $iCheckedCount = 0
	For $i = 0 To $iCount - 1
		If _GUICtrlListView_GetItemChecked ( $LISTVIEW1 , $i ) Then
			$aChecked[$iCheckedCount] = $i
			$iCheckedCount += 1
		EndIf
	Next

	If $iCheckedCount = 0 Then
		MsgBox ( 64 , "Close Marked" , "No profiles are marked. Use the checkboxes to mark profiles first." , 0 , $GGUIMAIN )
		Return
	EndIf

	; Confirm
	GUISetState ( @SW_DISABLE )
	Local $iAnswer = MsgBox ( 262196 , "Close Marked" , "Close " & $iCheckedCount & " marked profile(s)?" & @CRLF , 0 , $GGUIMAIN )
	GUISetState ( @SW_ENABLE )
	If $iAnswer <> 6 Then Return

	; Close in reverse order so indices don't shift
	For $j = $iCheckedCount - 1 To 0 Step -1
		Local $iIdx = $aChecked[$j]
		Local $sHwnd = _GUICtrlListView_GetItemText ( $LISTVIEW1 , $iIdx , 2 )
		Local $hCloseWnd = HWnd ( $sHwnd )

		If WinExists ( $hCloseWnd ) Then
			WinClose ( $hCloseWnd )
			; Brief wait then force kill if still there
			Sleep ( 200 )
...
Func CONFIGLOAD ( )
	...
	; Fix off-screen window position
	...
	$GHOTKEYFWD = HOTKEYPARSE ( IniRead ( $GCFGINI , "HOTKEYS" , "FORWARD" , "CTRL+SHIFT+RIGHT" ) )
	$GHOTKEYBCK = HOTKEYPARSE ( IniRead ( $GCFGINI , "HOTKEYS" , "BACKWARD" , "CTRL+SHIFT+LEFT" ) )
	$GHOTKEYTOP = HOTKEYPARSE ( IniRead ( $GCFGINI , "HOTKEYS" , "TOP" , "CTRL+SHIFT+UP" ) )
	$GHOTKEYSRT = HOTKEYPARSE ( IniRead ( $GCFGINI , "HOTKEYS" , "SORTTAB" , "CTRL+SHIFT+T" ) )
	$GHOTKEYSRP = HOTKEYPARSE ( IniRead ( $GCFGINI , "HOTKEYS" , "SORTPROFILE" , "CTRL+SHIFT+P" ) )
	$GHOTKEYGRPNEXT = HOTKEYPARSE ( IniRead ( $GCFGINI , "HOTKEYS" , "GROUPNEXT" , "" ) )
	$GHOTKEYGRPBACK = HOTKEYPARSE ( IniRead ( $GCFGINI , "HOTKEYS" , "GROUPBACK" , "" ) )
...
Func CONFIGSAVE ( )
	...
	EndIf
	IniWrite ( $GCFGINI , "HOTKEYS" , "FORWARD" , GUICtrlRead ( $INPUTHOTKEYFWD ) )
	IniWrite ( $GCFGINI , "HOTKEYS" , "BACKWARD" , GUICtrlRead ( $INPUTHOTKEYBCK ) )
	IniWrite ( $GCFGINI , "HOTKEYS" , "TOP" , GUICtrlRead ( $INPUTHOTKEYTOP ) )
	IniWrite ( $GCFGINI , "HOTKEYS" , "SORTTAB" , GUICtrlRead ( $INPUTHOTKEYSRT ) )
	IniWrite ( $GCFGINI , "HOTKEYS" , "SORTPROFILE" , GUICtrlRead ( $INPUTHOTKEYSRP ) )
	IniWrite ( $GCFGINI , "HOTKEYS" , "GROUPNEXT" , GUICtrlRead ( $INPUTHOTKEYGRPNEXT ) )
	IniWrite ( $GCFGINI , "HOTKEYS" , "GROUPBACK" , GUICtrlRead ( $INPUTHOTKEYGRPBACK ) )
	IniWrite ( $GCFGINI , "MAIN" , "HotkeysToggleExtra" , GUICtrlRead ( $INPUTEHKEY10 ) )
	For $I = 0 To UBound ( $GEXTRAHOTKEYCTRLS ) - 1
		For $Z = 0 To UBound ( $GEXTRAHOTKEYCTRLS , 2 ) - 1
...
Func WM_NOTIFY ( $HWND , $IMSG , $IWPARAM , $ILPARAM )
	...
	Case $HWNDLISTVIEW
		...
		EndIf
		; Detect column header click for sorting - set flag to prevent window popup
		If $ICODE = $LVN_COLUMNCLICK Then
			$GSORTINGINPROGRESS = True
			; Reset flag after a short delay using AdlibRegister
...
; ============================================
; DISCORD SCREENSHOT FUNCTIONS (Pure AutoIt - No Python needed)
; ============================================
Func SENDDISCORDSCREENSHOT ( $SCHANNEL )
	...
	; Split into chunks of 40 profiles per image
	Local $iMaxPerImage = 9999
	Local $iChunks = Ceiling ( $iCount / $iMaxPerImage )

	; Generate all chunk images first
	...
	; Send all images in a single Discord message
	...
	If $bResult And $sDiscordResponse <> "" Then
		; Parse first attachment URL from Discord JSON response
		; Look for "url":"https://cdn.discordapp.com/attachments/..."
	...
	; Log to Google Sheets if Sheet URL is configured
	...
	; Save screenshots locally if folder is set
	Local $sSaveFolder = GUICtrlRead ( $INPUTSCREENSHOTFOLDER )
	If $sSaveFolder <> "" Then
		; Create folder: SaveFolder/YYYY-MM-DD/que or prod/
		Local $sDate = @YEAR & "-" & @MON & "-" & @MDAY
		Local $sSavePath = $sSaveFolder & "\" & $sDate & "\" & $SCHANNEL
		DirCreate ( $sSavePath )

		; Build 12-hour time format for filename
...
; Save screenshot to local folder only (no Discord, no Sheets)
...
; Send all screenshots from today's prod folder to Discord + log profiles to Sheets
Func SENDPRODTODISCORD ( )
	...
	; Get all PNG files from today's prod folder
	...
	; Discord allows max 10 attachments per message - batch files in groups of 10
	Local $iBatchSize = 10
	Local $bResult = True
	...
	While $iBatch * $iBatchSize < $iFileCount
		...
		; Only include message text with the first batch
		Local $sBatchMsg = ""
	...
	; Extract screenshot URL from last Discord response
	Local $sScreenshotUrl = ""
	If $bResult And $sDiscordResponse <> "" Then
		Local $aUrlMatch = StringRegExp ( $sDiscordResponse , '"url"\s*:\s*"(https://cdn\.discordapp\.com/attachments/[^"]+)"' , 1 )
		If Not @error Then
	...
	; Log current profiles to Google Sheets
	Local $sSheetUrl = GUICtrlRead ( $INPUTSHEETURL )
	If $bResult And $sSheetUrl <> "" Then
		Local $iCount = _GUICtrlListView_GetItemCount ( $LISTVIEW1 )
		If $iCount > 0 Then
...
; Generate a profile table PNG image using GDI+
Func _GenerateProfileImage ( ByRef $aProfiles , $iCount , $sOutputFile )
	...
	; Create bitmap
	Local $aBmp = DllCall ( "gdiplus.dll" , "int" , "GdipCreateBitmapFromScan0" , "int" , $iImgWidth , "int" , $iImgHeight , "int" , 0 , "int" , 0x26200A , "ptr" , 0 , "ptr*" , 0 )
	...
	; Create brushes
	Local $hBrushWhite = _GDIPCreateSolidBrush ( 0xFFFFFFFF )
	Local $hBrushAltRow = _GDIPCreateSolidBrush ( 0xFFF0F0F0 )
	Local $hBrushHeader = _GDIPCreateSolidBrush ( 0xFFE0E0E0 )
	Local $hBrushBlack = _GDIPCreateSolidBrush ( 0xFF000000 )
	Local $hBrushGray = _GDIPCreateSolidBrush ( 0xFF999999 )
	Local $hPenBorder = _GDIPCreatePen ( 0xFF999999 , 1 )
	...
	Local $hFormat = $aSF[3]
	DllCall ( "gdiplus.dll" , "int" , "GdipSetStringFormatFlags" , "ptr" , $hFormat , "int" , 0x00001000 ) ; NoWrap
	...
	; Draw header text (with profile count)
	...
	DllStructSetData ( $tCLSID , "Data1" , 0x557CF406 )
	DllStructSetData ( $tCLSID , "Data2" , 0x1A04 )
	DllStructSetData ( $tCLSID , "Data3" , 0x11D3 )
...
; Send text-only message to Discord (no screenshot)
Func SENDDISCORDTEXTONLY ( $SCHANNEL )
	...
	; Poll for response while keeping GUI alive
...
; Upload file to Discord webhook via multipart/form-data POST
Func _DiscordWebhookUpload ( $sWebhookUrl , $sFilePath , $sContent , $sUsername = "MLM Manager" )
	; Read file into binary
	Local $hFile = FileOpen ( $sFilePath , 16 ) ; Binary mode
	...
	Local $bTop = StringToBinary ( $sBodyTop , 4 ) ; UTF8
...
Func _DiscordWebhookUploadMulti ( $sWebhookUrl , ByRef $aFiles , $iFileCount , $sContent , $sUsername = "MLM Manager" )
	...
	; Build file parts and calculate total size
	; First pass: read all files and build headers
...
; Log profile data to Google Sheets via Apps Script webhook
Func _LogToGoogleSheets ( $sSheetUrl , $sVAName , ByRef $aProfiles , $iCount , $sChannel , $sMessage , $sScreenshotUrl )
	...
	For $i = 0 To $iCount - 1
		If $i > 0 Then $sRows &= ","
		; Escape any quotes in profile/tab text
	...
	; Include channel so Apps Script knows which sheet tab to use
	Local $sSheetTab = "PRODUCTION"
	If StringLower ( $sChannel ) = "que" Then $sSheetTab = "QUEUE"
	Local $sJSON = '{"sheet":"' & $sSheetTab & '","rows":' & $sRows & '}'

	; POST to Apps Script using MSXML2 - handles redirects reliably
	; Apps Script processes the data then returns 302 redirect
	...
	Local $iStatus = $oHTTP.Status
	; Apps Script processes data on POST, any non-server-error means data was received
	; 200 = success after redirect, 302 = redirect (data processed), 405 = redirect page issue (data still processed)
...
Func LOADDISCORDSETTINGS ( )
	...
	; Auto-create Screenshots folder in the MLM directory if not set
	...
	For $iRow = 0 To $iRows - 1
		For $iCol = 0 To $iCols - 1
			...
			If $sHwnd <> "" Then
				...
				DllCall ( "user32.dll" , "bool" , "ShowWindow" , "hwnd" , $hWnd , "int" , 1 ) ; SW_SHOWNORMAL
				DllCall ( "user32.dll" , "bool" , "SetWindowPos" , "hwnd" , $hWnd , "hwnd" , 0 , "int" , $iX , "int" , $iY , "int" , $iWidth , "int" , $iHeight , "uint" , 0x0040 )
			EndIf
...
Func OPENURLALL ( )
	...
	; Extract domain from URL for comparison (e.g., "google" from "google.com")
	...
	; Copy URL to clipboard once, then paste into each browser
	ClipPut ( $sURL )

	For $i = 0 To $iCount - 1
		; Check if Tab column already has THIS URL/domain open - skip if it does
		Local $sTab = _GUICTRLLISTVIEW_GETITEMTEXT ( $LISTVIEW1 , $i , 1 )
		Local $sTabLower = StringLower ( $sTab )
		; Skip if profile already has this specific URL/domain open
		...
		If Not $bHasThisURL Then
			...
			If $sHwnd <> "" Then
				...
				Send ( "^t" )
				Sleep ( 300 )
				; Paste URL into the new tab's address bar (already focused)
				Send ( "^v" )
				Sleep ( 150 )
				Send ( "{ENTER}" )
				Sleep ( 200 )
...
; Resize all profile windows to specified width/height and stack them at top-left (0,0)
	...
	For $i = 0 To $iCount - 1
		...
		If $sHwnd <> "" Then
			...
			If WinExists ( $hWnd ) Then
				; First restore from maximized state if needed
				WinSetState ( $hWnd , "" , @SW_RESTORE )
				; Move to top-left (0,0) and resize
...
; Switch to a group - show and position only the windows in that group, minimize the rest
Func SWITCHGROUP ( $iGroupIndex )
	...
	; Calculate which windows belong to this group
	Local $iStart = $iGroupIndex * $iGroupSize
	Local $iEnd = $iStart + $iGroupSize - 1
	If $iEnd >= $iCount Then $iEnd = $iCount - 1

	; Update group status label
	...
	For $iB = 0 To 25
		If $iB = $iGroupIndex Then
			GUICtrlSetBkColor ( $GROUPBUTTONS[$iB] , 0x4CAF50 )
			GUICtrlSetColor ( $GROUPBUTTONS[$iB] , 0xFFFFFF )
		Else
			GUICtrlSetBkColor ( $GROUPBUTTONS[$iB] , 0xF0F0F0 )
			GUICtrlSetColor ( $GROUPBUTTONS[$iB] , 0x000000 )
		EndIf
	Next
	...
	; Step 2: Show and position group windows in grid
	Local $iIdx = 0
	For $i = $iStart To $iEnd
		Local $sHwnd = _GUICTRLLISTVIEW_GETITEMTEXT ( $LISTVIEW1 , $i , 2 )
		If $sHwnd <> "" Then
			...
			; Force window out of minimized/maximized state using ShowWindow API
			DllCall ( "user32.dll" , "bool" , "ShowWindow" , "hwnd" , $hWnd , "int" , 1 ) ; SW_SHOWNORMAL
			Sleep ( 50 )
			; Move and resize to exact position
			DllCall ( "user32.dll" , "bool" , "SetWindowPos" , "hwnd" , $hWnd , "hwnd" , 0 , "int" , $iX , "int" , $iY , "int" , $iWidth , "int" , $iHeight , "uint" , 0x0040 ) ; SWP_SHOWWINDOW
		EndIf
		$iIdx += 1
	Next
...
; Switch to a group from Main tab - show windows in that group, minimize the rest
; Uses settings from Positioner tab for grid positioning
Func GroupMoveNext ( )
	; Calculate total groups based on current profile count and grid size
	Local $iCols = Int ( GUICtrlRead ( $INPUTCOLS ) )
	Local $iRows = Int ( GUICtrlRead ( $INPUTROWS ) )
	If $iCols < 1 Then $iCols = 1
	If $iRows < 1 Then $iRows = 1
	...
	If $iCount = 0 Then Return
	Local $iTotalGroups = Ceiling ( $iCount / $iGroupSize )
	; Move to next group (wrap around to A)
...
Func GroupMoveBack ( )
	...
	If $iCount = 0 Then Return
	Local $iTotalGroups = Ceiling ( $iCount / $iGroupSize )
	; Move to previous group (wrap around to last)
...
Func SWITCHGROUPMAIN ( $iGroupIndex )
	...
	; Highlight active main group button
	For $iB = 0 To 25
		If $iB = $iGroupIndex Then
			GUICtrlSetBkColor ( $MAINGROUPBUTTONS[$iB] , 0x4CAF50 )
			GUICtrlSetColor ( $MAINGROUPBUTTONS[$iB] , 0xFFFFFF )
		Else
			GUICtrlSetBkColor ( $MAINGROUPBUTTONS[$iB] , 0xF0F0F0 )
			GUICtrlSetColor ( $MAINGROUPBUTTONS[$iB] , 0x000000 )
		EndIf
	Next
	$GACTIVEGROUP = $iGroupIndex
...
Func _ShowSplashScreen()
	; Try BMP first (most compatible), then JPG, then PNG
	Local $sSplashPath = @ScriptDir & "\MLManagerData\splash.bmp"
	If Not FileExists($sSplashPath) Then $sSplashPath = @ScriptDir & "\MLManagerData\splash.jpg"
	If Not FileExists($sSplashPath) Then $sSplashPath = @ScriptDir & "\MLManagerData\splash.png"
	If Not FileExists($sSplashPath) Then Return

	; Size of splash (circular)
	Local $iSize = 200
	...
	; Create borderless popup window with black background
	Local $hSplash = GUICreate("MLM Splash", $iSize, $iSize, $iX, $iY, 0x80000000, 0x00080000) ; WS_POPUP, WS_EX_LAYERED
	GUISetBkColor(0x000000, $hSplash)

	; Create picture control with the image - use GUICtrlSetImage for better compatibility
	Local $idPic = GUICtrlCreatePic("", 0, 0, $iSize, $iSize)
	GUICtrlSetImage($idPic, $sSplashPath)

	; Make window round (circular region)
